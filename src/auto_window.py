...
from utils import clean_outputdir, copy_path_files
...
def copy_param_files(taglist, inputbase, outputbase):
    ...
    for tag in taglist:
        inputfiles = glob.glob(os.path.join(inputbase, "%s.*.yml" % tag))
        for _file in inputfiles:
            shutil.copy(_file, outputdir)

def copy_job_script(template_dir, outputdir):
    origin_file = os.path.join(template_dir, "window_selection_asdf.py")
    outfile = os.path.join(outputdir, "window_selection_asdf.py")
...
def modify_scripts_for_serial_run(template_dir, workdir, cmtlist, job_idx,
    ...
    inputfile = os.path.join(template_dir, "job_window_one_event.bash")
    nnodes_per_event = config["nnodes_per_event"]
    nprocs_per_node = config["nprocs_per_node"]
    nprocs = nnodes_per_event * nprocs_per_node
    outputfile = os.path.join(workdir, "job_window_one_event.bash")
    ...
    inputfile = os.path.join(template_dir, "job_window.bash")
    cmtlist_file = "cmtlist"
    outputfile = os.path.join(workdir, "job_window.bash")
...
def modify_scripts_for_simul_run(template_dir, workdir, cmtlist, job_idx,
    ...
    for cmt_idx in range(len(cmtlist)):
        outputfile = os.path.join(workdir, "job_window_one_event.%d.bash"
                                  % cmt_idx)
        modify_one_event_script(inputfile, nprocs, config["taglist"],
                                outputfile)
...
def modify_job_script(inputfile, nnodes, walltime, cmtlist_file, job_idx,
    ...
    for line in content:
        ...
        # modify for serial
        if mode == "serial":
            if line.startswith("  bash job_window_one_event"):
                line = "  bash job_window_one_event.bash $event"
...
def prepare_one_job(job_id, cmtlist, config):
    ...
    outputdir = os.path.join(outputbase, "job_window_%d" % job_id)
    if not os.path.exists(outputdir):
        os.makedirs(outputdir)
